...
const TelemedicinePage = () => {
  ...
  const fetchPlans = async () => {
    try {
      const response = await fetch(`${API}/telemedicine-plans`, { headers });
  ...
  const fetchMyPlans = async () => {
    try {
      const response = await fetch(`${API}/my-telemedicine-plans`, { headers });
      const data = await response.json();
      if (data.success) {
  ...
  const handlePurchase = async (planId) => {
    ...
    try {
      const response = await fetch(`${API}/telemedicine-plans/${planId}/purchase`, {
        method: 'POST',
        headers
      });
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-100">
        <div className="bg-white shadow-sm border-b">
          <div className="max-w-md mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <h1 className="text-lg font-bold text-gray-800">Telemedicina</h1>
            <div></div>
          </div>
        </div>
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-[#2A3618]' : 'bg-[#EEEEEE]'}`}>
      ...
      <div className={`shadow-sm border-b ${isDarkMode ? 'bg-[#3F5123] border-[#005B9C]' : 'bg-[#FFFFFF] border-[#005B9C]'}`}>
        <div className="max-w-md mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <h1 className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-[#333333]'}`}>Telemedicina</h1>
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        ...
        <Card className="bg-gradient-to-r from-red-600 to-pink-600 text-white">
          <CardContent className="p-6 text-center">
            <Stethoscope className="w-16 h-16 mx-auto mb-4 text-red-100" />
            <h2 className="text-2xl font-bold mb-2">Telemedicina Transmill</h2>
            <p className="text-red-100 mb-4">
              Cuide da sua saúde e ganhe cashback
            </p>
            <div className="flex items-center justify-center gap-2">
              <Badge className="bg-white/20 text-white">
                🎁 Cashback em todas as consultas
              </Badge>
            </div>
          </CardContent>
        </Card>
        ...
        {myPlans.length > 0 && (
          <Card className="border-orange-200">
            ...
            <CardContent className="space-y-3">
              {myPlans.filter(plan => plan.is_active).slice(0, 2).map((plan) => (
                <div key={plan.id} className="p-3 bg-transmill-gold/10 rounded-lg border border-transmill-gold">
                  ...
                  <div className="text-sm text-orange-700">
                    {plan.consultations_remaining && (
                      <div>🩺 {plan.consultations_remaining} consultas restantes</div>
                    )}
          ...
          {plans.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-600">Em breve teremos planos de telemedicina para você!</p>
              </CardContent>
            </Card>
          ) : (
            plans.map((plan) => (
              <Card 
                key={plan.id} 
                className="border-2 border-transparent hover:border-red-200 transition-all duration-200"
              >
                <CardContent className="p-0">
                  {/* Imagem do plano */}
                  {plan.image_url && (
                    <div className="w-full h-32 bg-gradient-to-r from-red-500 to-pink-500 rounded-t-lg overflow-hidden">
                      <img 
                        src={plan.image_url} 
                        alt={plan.name}
                        ...
                        onError={(e) => {
                          ...
                          e.target.parentElement.className = 'w-full h-32 bg-gradient-to-r from-red-500 to-pink-500 rounded-t-lg overflow-hidden flex items-center justify-center';
                        }}
                      />
                    </div>
                  )}
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-gray-500">por {plan.validity_days} dias</div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2 mb-4">
                      {plan.consultations_included && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">{plan.consultations_included} consultas incluídas</span>
                        </div>
                      )}
                      
                      <div className="flex items-center gap-2 text-sm">
                        <Clock className="text-red-600" size={16} />
                        <span>Validade: {plan.validity_days} dias</span>
                      </div>

                      <div className="flex items-center gap-2 text-sm">
                        <Star className="text-red-600" size={16} />
                        <span>Cashback: {plan.formatted_cashback}</span>
                      </div>
                    </div>
                    ...
                    {plan.features && plan.features.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-sm font-semibold text-gray-800 mb-2">Incluso:</h4>
                    ...
                    {plan.specialties && plan.specialties.length > 0 && (
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {plan.specialties.map((specialty, index) => (
                    ...
                    <div className="bg-gradient-to-r from-orange-50 to-red-50 p-3 rounded-lg mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-semibold text-orange-800">Benefícios Transmill</span>
                      </div>
                      <div className="space-y-1 text-xs text-orange-700">
                        <div className="flex items-center gap-1">
                          <Star className="text-orange-500" size={12} />
                          <span>Cashback em todas as consultas</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Star className="text-orange-500" size={12} />
                        ...
                        <div className="flex items-center gap-1">
                          <Star className="text-orange-500" size={12} />
                          <span>Atendimento rápido e online</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Botão de compra */}
                    <Button 
                      className="w-full bg-transmill-olive hover:bg-transmill-olive-dark text-white py-3"
        ...
        <Card className="bg-transmill-gold/10 border-transmill-gold">
          <CardContent className="p-4">
            <h4 className="font-semibold text-transmill-olive mb-3">🩺 Como funciona?</h4>
            <div className="space-y-2 text-sm text-red-700">
              <div>• Escolha o plano ideal para você</div>
              <div>• Pague com o saldo da sua conta Transmill</div>
              <div>• Agende suas consultas online</div>
              <div>• Ganhe cashback em todas as consultas</div>
            </div>
          </CardContent>
        </Card>
      </div>
