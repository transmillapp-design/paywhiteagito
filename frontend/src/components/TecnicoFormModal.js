...
import { X, Building2, User, DollarSign, Percent, Wrench } from 'lucide-react';
...
const TecnicoFormModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    telefone_contato: '',
    observacao: '',
    area_atuacao: '', // CAMPO NOVO PARA TÉCNICO
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...formData,
        natureza,
        user_type: 'labelview_tecnico',
        is_labelview_tecnico: true
      };
      
      const response = await axios.post(`${API}/labelview/tecnicos`, dataToSend, {
        headers: {
      ...
      toast.error(error.response?.data?.detail || 'Erro ao cadastrar técnico');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-[#1a59ad] text-white p-6 flex items-center justify-between border-b-4 border-[#2fa31c]">
          <div className="flex items-center gap-3">
            <Wrench size={32} />
            <div>
              <h2 className="text-2xl font-bold">Novo Técnico</h2>
              <p className="text-sm opacity-90">Cadastro de Técnico - Pessoa Física ou Jurídica</p>
            </div>
          </div>
            ...
            <>
              <div className="border-t-4 border-[#2fa31c] pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* CAMPO NOVO - ÁREA DE ATUAÇÃO */}
                  <div className="md:col-span-2">
                    <Label>Área de Atuação</Label>
                    <Input
                      value={formData.area_atuacao}
                      onChange={(e) => setFormData({...formData, area_atuacao: e.target.value})}
                      placeholder="Ex: Instalação de rastreadores, Manutenção preventiva, Suporte técnico"
                      maxLength={200}
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Especifique a área de atuação do técnico (máximo 200 caracteres)
                    </p>
                  </div>
